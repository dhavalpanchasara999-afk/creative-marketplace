...
const INITIAL_PRODUCTS = [
    {
        ...
        tags: "graphics, design, bundle, psd, vector",
        reviews: [
            { name: "Anand Verma", text: "Got the Google Drive link immediately. Incredibly rich collection!", rating: 5 },
            { name: "Sneha Reddy", text: "The vector templates are high-quality, saves me hours of work.", rating: 5 }
        ]
    },
    {
        id: 2,
        ...
        tags: "video, effects, transitions, luts, youtube",
        reviews: [
            { name: "Md Rizwan", text: "The LUT profiles alone are worth the price. Must have for video editors.", rating: 5 }
        ]
    },
    {
        id: 3,
        title: "2 Lakh+ Vector T-Shirt Designs Bundle",
        slug: "tshirt-designs-bundle",
        description: "Scale your e-commerce print-on-demand store with over 200,000+ vector T-shirt design illustrations. Compatible with Photoshop, Illustrator, and Canva. Fully categorized into trending niches like sports, abstract, typography, and vintage styles.",
        category: "Templates",
        rating: 5,
        salePrice: 299,
        originalPrice: 8999,
        ...
        tags: "tshirt, template, print, svg, vector",
        reviews: [
            { name: "Santana Asharjee", text: "Great collection, easy printing resolution vector layers.", rating: 5 }
        ]
    },
    {
        id: 4,
        title: "Premium Fonts Mega Vault",
        slug: "premium-fonts-vault",
        description: "The ultimate 1.5 Lakh+ premium typeface collection. Includes high-end serif, modern sans-serif, brush-script, display, and futuristic font categories. Perfect for branding packages, website designs, and professional marketing campaigns.",
        category: "Fonts",
        rating: 5,
        salePrice: 199,
        originalPrice: 10999,
        ...
        tags: "fonts, typography, branding, ttf, otf",
        reviews: [
            { name: "Manoj Tiwari", text: "Very diverse clean styles. Saved a ton of subscription money.", rating: 5 }
        ]
    },
    {
        id: 5,
        title: "Midjourney & ChatGPT AI Prompt Guide",
        slug: "ai-prompt-vault",
        description: "A comprehensive digital vault containing 10,000+ copy-paste high-performing AI prompts for Midjourney art creation, ChatGPT copywriting, SEO blog building, and marketing campaign automations. Maximize your digital agency output.",
        category: "AI Prompts",
        rating: 4,
        salePrice: 149,
        originalPrice: 4999,
        ...
        tags: "prompts, ai, midjourney, chatgpt, templates",
        reviews: []
    },
    {
        id: 6,
        title: "Modern SaaS Website Landing Page Templates",
        slug: "saas-landing-templates",
        description: "Fully responsive, high-converting React, Next.js, and HTML landing page designs. Integrated clean styling system, glassmorphism cards, glowing gradients, and smooth scroll animations. Completely customizable and optimized for SEO performance.",
        category: "Templates",
        rating: 5,
        salePrice: 499,
        originalPrice: 15999,
        ...
        tags: "templates, nextjs, html, saas, react",
        reviews: []
    },
    {
        id: 7,
        title: "E-Book Empire: 1500+ Niche Guides",
        slug: "ebook-empire-niche-guides",
        description: "Huge digital library containing 1,500+ best-selling eBooks in high-performing domains like startup finance, passive income building, stock market trading, habits optimization, mental self-growth, and online marketing strategy.",
        category: "E-books",
        rating: 5,
        salePrice: 349,
        originalPrice: 9999,
        ...
        tags: "ebooks, pdf, books, self-growth, business",
        reviews: []
    },
    {
        id: 8,
        title: "Complete Freelance Web Agency Course",
        slug: "freelance-web-agency-course",
        description: "Step-by-step video training course showing you how to build, scale, and automate a 6-figure freelance web design agency. Covers lead generation tricks, cold outreach templates, high-ticket sales strategies, and project deliverables optimization.",
        category: "Courses",
        rating: 5,
        salePrice: 599,
        originalPrice: 18999,
        ...
        tags: "course, agency, freelance, marketing, learning",
        reviews: []
    }
];
...
function applyCmsBranding() {
    const name = document.getElementById('cmsBrandName').value;
    const domain = document.getElementById('cmsBrandDomain').value;
    const email = document.getElementById('cmsSupportEmail').value;
    const logoUrl = document.getElementById('cmsLogoUrl').value;
    ...
    // Apply branding titles
    document.querySelectorAll('.logo-part-1').forEach(el => el.textContent = name.slice(0, 4));
    document.querySelectorAll('.logo-part-2').forEach(el => el.textContent = name.slice(4));
    document.querySelectorAll('.logo-domain').forEach(el => el.textContent = `.${domain.split('.').slice(-1)[0]}`);

    // Dynamic brand logo images
    if (logoUrl) {
        document.querySelectorAll('.brand-logo-img').forEach(el => el.src = logoUrl);
    }
...
function startScarcityTimer() {
    ...
    // Dynamic countdown timer targeting cms config hours
    let hoursLeft = CMS_CONFIG.countdownHours;
    let secondsLeft = hoursLeft * 3600;
    ...
    const updateTimerUI = () => {
        ...
        daysBox.textContent = pad(d);
        hoursBox.textContent = pad(h);
        minsBox.textContent = pad(m);
        secsBox.textContent = pad(s);
    };
    ...
    timerCountdownInterval = setInterval(() => {
        if (secondsLeft <= 0) {
            secondsLeft = CMS_CONFIG.countdownHours * 3600;
        } else {
            secondsLeft--;
        }
        updateTimerUI();
...
function loadProductDetails(productId) {
    ...
    setTimeout(() => {
        ...
        let relatedHtml = '';
        const related = PRODUCTS.filter(p => p.category === prod.category && p.id !== prod.id).slice(0, 3);
        if (related.length > 0) {
            relatedHtml = `
...
function handleProductFormSubmit(e) {
    ...
    if (id) {
        // EDIT mode
        const pId = Number(id);
        PRODUCTS = PRODUCTS.map(p => p.id === pId ? {
            ...p,
            title, description: desc, category: cat, rating: rat, salePrice: sale, originalPrice: orig, thumbnail: thumb, downloadUrl: zip, tags
        } : p);
        ...
        PRODUCTS.push({
            id: newId,
            title, slug, description: desc, category: cat, rating: rat, salePrice: sale, originalPrice: orig, thumbnail: thumb, downloadUrl: zip, tags, reviews: []
        });
